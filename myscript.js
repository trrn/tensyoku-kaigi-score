function patch() {
    ...
    jQuery(this).find("tr").each(function() {
      ...
      if (th) {
        ...
        if (em.length) { // total score for graph
          em_score = em;
        } else if (th.text().match(/^\d.0$/)) { // score
          var score = th.text()|0;
  ...
  jQuery("p[class^='star01_'], span[class^='star01_'], li.point").each(function() {
    var score = (jQuery(this).find("em").text().match(/(\d.\d)/)||[])[0];
    if (score) {
      var klass = 'star01_' + (Math.round(parseFloat(score)*2)/2)*10;
      jQuery(this).removeClass(function(index, css) {
        return (css.match(/star01_\S+/g)||[])[0];
      }).addClass(klass);
    }
  });
}
